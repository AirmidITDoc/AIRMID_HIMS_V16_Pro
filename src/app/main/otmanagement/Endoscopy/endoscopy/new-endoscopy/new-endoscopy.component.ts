...
@Component({
  selector: 'app-new-endoscopy',
  templateUrl: './new-endoscopy.component.html',
  styleUrls: ['./new-endoscopy.component.scss'],
  encapsulation: ViewEncapsulation.None,
  animations: fuseAnimations
})
export class NewEndoscopyComponent implements OnInit {
  ...
  isLoading: string = '';
  Prefix :any;
  registerObj1 = new OTEndoscopydetail({});
  OPDate:any;
IsPathRad:any;
PatientName:any ='';
OPIP:any='';
...
public dateValue: Date = new Date();
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<NewEndoscopyComponent>,
...
// if(this.Adm_Vit_ID){
    if (!OTEndoscopyBookingID) {
      ...
      console.log(m_data);
      this._OtManagementService.EndoscopyrequestInsert(m_data).subscribe(response => {
        if (response) {this._OtManagementService
          Swal.fire('Congratulations !', 'OT Endoscopy  Data save Successfully !', 'success').then((result) => {
          ...
          Swal.fire('Error !', 'Endoscopy Data  not saved', 'error');
        }
      ...
      console.log(m_data1);
      this._OtManagementService.EndoscopyrequestUpdate(m_data1).subscribe(response => {
        if (response) {
          Swal.fire('Congratulations !', 'Endoscopy Data Updated Successfully !', 'success').then((result) => {
            if (result.isConfirmed) {
              this._matDialog.closeAll();
            }
          });
